...
public class UsersService {
    ...
    public Result login(String ssn, String password){
    	...
    	//如果查询结果不为空
		if (user != null) {
			System.out.println(user.getPassword() + " is from mapper " + password + " is the password from frontend");
	...
	//根据token获取user
		public Users getUsersByToken(String token){
			UsersToken usersToken = usersTokenMapper.selectByToken(token);
			Users user = usersMapper.selectById(usersToken.getId());
			return user;
		}
